...
public class YearlySubscriptionTest extends base {
	...
	@Test(priority = 1)
	public void yearlySubscriptionTest() throws InterruptedException {

		Thread.sleep(3000);
		p.yearlySub().click();
		wait.until(ExpectedConditions.visibilityOfElementLocated(p.business));
		String s4 = p.payAsYouGoAmount().getText();
		log.info("Yearly - pay as you go price fetched:-{}", s4);
		String s5 = p.businessAmount().getText();
		log.info("Yearly   - Business price fetched:-${}/mo", splitFun1(s5));
		Thread.sleep(2000);
		String s6 = p.businessBilledAmount().getAttribute("innerHTML");
		log.info("Yearly   - Business billed price fetched:-${}/qa", splitFun2(s6));
		String s7 = p.premiumAmount().getText();
		log.info("Yearly   - premium  price fetched:-${}/mo", splitFun1(s7));
		Thread.sleep(2000);
		String s8 = p.premiumBilledAmount().getAttribute("innerHTML");
		log.info("Yearly   - premium billed price fetched:-${}/qa", splitFun2(s8));
		String s9 = p.businessAmountsave().getAttribute("innerHTML").substring(14);

		log.info("Yearly   - Business Saved amount fetched:-${}", s9);
		String s10 = p.premiumAmountsave().getAttribute("innerHTML").substring(14);
		log.info("Yearly   - Premium Saved amount fetched:-${}", s10);
		String s11 = p.businessBilledWithoutSave().getAttribute("innerHTML");

		log.info("Yearly   - Business Billed amount without saving fetched:-${}", s11);
		String s12 = p.premiumBilledWithoutSave().getAttribute("innerHTML");
		log.info("Yearly   - Premium Billed amount without saving fetched:-${}", s12);
		...
		if (f1 * 12 == f2) {
			log.info("Yearly  ----Test Passed-- Business billed Price  of   ${}",t2);
			Assert.assertTrue(true);

		} else {
			log.error("Yearly  -----Test failed -----Business billed Price for value ${} on place of ${}", f1 * 12, f2);
		...
		if (f3 * 12 == f4) {
			log.info("Yearly  ----Test Passed----Premium billed Price  of amount ${}",t4);
			Assert.assertTrue(true);

		} else {
			log.error("Yearly  -----Test failed---Premium billed Price for value ${} on place of ${}", f3 * 12, f4);
			Assert.assertTrue(false);
		}
		float k1=Float.parseFloat(s11);
		
		if (Math.round(k1-Float.parseFloat(t2)) == Math.round(Float.parseFloat(s9))) {
			log.info("Yearly  -----Test Passed---Business billed Saving ${}  ",s9);
			Assert.assertTrue(true);

		} else {
			log.error("Yearly  -----Test Failed---Business billed Saving ${}  ",s9);
			Assert.assertTrue(false);
		}
		float k2=Float.parseFloat(s12);
		if (Math.round(k2-Float.parseFloat(t4)) == Math.round(Float.parseFloat(s10))) {
			log.info("Yearly  ----Test Passed---Premium billed Saving ${}  ",s10);
			Assert.assertTrue(true);

		} else {
			log.error("Yearly  ----Test failed-----Premium  billed Saving ${} ",s10);
			Assert.assertTrue(false);
		}
